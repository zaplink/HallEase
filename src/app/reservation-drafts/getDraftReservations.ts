...
export async function getDraftReservations(): Promise<UnifiedReservationRow[]> {
	...
	try {
		...
		// Get only draft reservations for this user
		...
		if (reserveError) {
			console.error('Error fetching draft reserves:', reserveError);
			return [];
		}
		...
		const unifiedRows: UnifiedReservationRow[] = reserves.map((reserve) => {
			...
			return {
				...
				modifiedDate: reserve.modified_date || '',
				modifiedTime: reserve.modified_time || '',
			};
		});

		return unifiedRows;
	} catch (error) {
		console.error('Unexpected error fetching draft reservations:', error);
		return [];
	}
}
